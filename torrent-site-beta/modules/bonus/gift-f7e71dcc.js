var J = Object.defineProperty,
    G = Object.defineProperties;
var F = Object.getOwnPropertyDescriptors;
var R = Object.getOwnPropertySymbols;
var W = Object.prototype.hasOwnProperty,
    K = Object.prototype.propertyIsEnumerable;
var T = (s, e, t) => e in s ? J(s, e, {
        enumerable: !0,
        configurable: !0,
        writable: !0,
        value: t
    }) : s[e] = t,
    g = (s, e) => {
        for (var t in e || (e = {})) W.call(e, t) && T(s, t, e[t]);
        if (R)
            for (var t of R(e)) K.call(e, t) && T(s, t, e[t]);
        return s
    },
    P = (s, e) => G(s, F(e));
var B = (s, e, t) => (T(s, typeof e != "symbol" ? e + "" : e, t), t);
import {
    _ as p
} from "./main-45511cec.js";
var Pa = typeof globalThis != "undefined" ? globalThis : typeof window != "undefined" ? window : typeof global != "undefined" ? global : typeof self != "undefined" ? self : {};

function O(s) {
    return s && s.__esModule && Object.prototype.hasOwnProperty.call(s, "default") ? s.default : s
}

function Qa(s) {
    if (s.__esModule) return s;
    var e = s.default;
    if (typeof e == "function") {
        var t = function a() {
            return this instanceof a ? Reflect.construct(e, arguments, this.constructor) : e.apply(this, arguments)
        };
        t.prototype = e.prototype
    } else t = {};
    return Object.defineProperty(t, "__esModule", {
        value: !0
    }), Object.keys(s).forEach(function(a) {
        var n = Object.getOwnPropertyDescriptor(s, a);
        Object.defineProperty(t, a, n.get ? n : {
            enumerable: !0,
            get: function() {
                return s[a]
            }
        })
    }), t
}
var i = globalThis["react/jsx-runtime"],
    h = globalThis.react;
const Y = O(h);
var o = globalThis["@bc/ui"];

function H({
    children: s
}) {
    return i.jsx(o.I18nProvider, {
        namespace: "bonus",
        locales: Object.assign({
            "../locales/ar-SA/bonus.json": () => p(() =>
                import ("./bonus-e57a59d2.js"), []).then(e => e.default),
            "../locales/bn-BD/bonus.json": () => p(() =>
                import ("./bonus-e27427ac.js"), []).then(e => e.default),
            "../locales/de-DE/bonus.json": () => p(() =>
                import ("./bonus-dc36fca1.js"), []).then(e => e.default),
            "../locales/en-IN/bonus.json": () => p(() =>
                import ("./bonus-ceeeab1d.js"), []).then(e => e.default),
            "../locales/en-US/bonus.json": () => p(() =>
                import ("./bonus-3a61785a.js"), []).then(e => e.default),
            "../locales/es-ES/bonus.json": () => p(() =>
                import ("./bonus-2c53fa4f.js"), []).then(e => e.default),
            "../locales/fa-IR/bonus.json": () => p(() =>
                import ("./bonus-2068e4c1.js"), []).then(e => e.default),
            "../locales/fi-FI/bonus.json": () => p(() =>
                import ("./bonus-ffb51c15.js"), []).then(e => e.default),
            "../locales/fil-PH/bonus.json": () => p(() =>
                import ("./bonus-37561564.js"), []).then(e => e.default),
            "../locales/fr-FR/bonus.json": () => p(() =>
                import ("./bonus-0ba247d8.js"), []).then(e => e.default),
            "../locales/hi-IN/bonus.json": () => p(() =>
                import ("./bonus-eca0115d.js"), []).then(e => e.default),
            "../locales/id-ID/bonus.json": () => p(() =>
                import ("./bonus-6616dcc3.js"), []).then(e => e.default),
            "../locales/it-IT/bonus.json": () => p(() =>
                import ("./bonus-2cdc01a5.js"), []).then(e => e.default),
            "../locales/ja-JP/bonus.json": () => p(() =>
                import ("./bonus-1c1ea713.js"), []).then(e => e.default),
            "../locales/ko-KR/bonus.json": () => p(() =>
                import ("./bonus-fbfadfad.js"), []).then(e => e.default),
            "../locales/mr-IN/bonus.json": () => p(() =>
                import ("./bonus-35b710e3.js"), []).then(e => e.default),
            "../locales/ms-MY/bonus.json": () => p(() =>
                import ("./bonus-b0f77caf.js"), []).then(e => e.default),
            "../locales/my-MM/bonus.json": () => p(() =>
                import ("./bonus-cb66ff72.js"), []).then(e => e.default),
            "../locales/nl-NL/bonus.json": () => p(() =>
                import ("./bonus-21b0973f.js"), []).then(e => e.default),
            "../locales/pl-PL/bonus.json": () => p(() =>
                import ("./bonus-98fbb79a.js"), []).then(e => e.default),
            "../locales/pt-BR/bonus.json": () => p(() =>
                import ("./bonus-33935991.js"), []).then(e => e.default),
            "../locales/ru-RU/bonus.json": () => p(() =>
                import ("./bonus-91af95af.js"), []).then(e => e.default),
            "../locales/ta-IN/bonus.json": () => p(() =>
                import ("./bonus-b6979425.js"), []).then(e => e.default),
            "../locales/te-IN/bonus.json": () => p(() =>
                import ("./bonus-4ba477db.js"), []).then(e => e.default),
            "../locales/th-TH/bonus.json": () => p(() =>
                import ("./bonus-ce18399a.js"), []).then(e => e.default),
            "../locales/tr-TR/bonus.json": () => p(() =>
                import ("./bonus-31b4bd14.js"), []).then(e => e.default),
            "../locales/uk-UA/bonus.json": () => p(() =>
                import ("./bonus-33d48e38.js"), []).then(e => e.default),
            "../locales/ur-PK/bonus.json": () => p(() =>
                import ("./bonus-47d6ccb8.js"), []).then(e => e.default),
            "../locales/vi-VN/bonus.json": () => p(() =>
                import ("./bonus-d6f6fd71.js"), []).then(e => e.default)
        }),
        children: s
    })
}

function X(s) {
    return e => i.jsx(H, {
        children: i.jsx(s, g({}, e))
    })
}
const Z = 22,
    Q = 50,
    _ = {
        first: {
            bonusLogs: [],
            expiredTime: 0,
            maxBonusRatio: [3, 1.8, 2.4, 3, 3.6],
            cashbackBonus: [],
            freeSpins: null,
            newbieTaskBonus: null,
            lotteryBonus: null,
            currencyName: ""
        },
        second: 0
    },
    $ = {
        depositBonus: 0,
        freeSpinBonus: 0,
        levelUpBonus: 0,
        luckySpinBonus: 0,
        monthlyBonus: 0,
        questsBonus: 0,
        rechargeBonus: 0,
        rollCompetitionBonus: 0,
        sportsWeeklyBonus: 0,
        weeklyBonus: 0
    },
    ee = {
        first: {
            totalClaimed: 0,
            totalGeneralBonus: 0,
            totalSpecialBonus: 0,
            totalVipBonus: 0
        },
        second: 0
    },
    te = {
        first: {
            currentWager: "0",
            expireTimeEpochSecond: 0,
            levelLimit: 22,
            startTimeEpochSecond: 0,
            targetWager: "0"
        },
        second: 0
    },
    z = {
        first: {
            currentWager: 0,
            expireTimeEpochSecond: 0,
            startTimeEpochSecond: 0,
            levelLimit: 22,
            targetWager: 0
        },
        second: 0
    },
    se = {
        first: {
            rewardCurrency: "",
            rewardsAmount: 0,
            newbieTaskExpireTime: 0,
            rewards: [],
            dailyTasks: [],
            weeklyTasks: []
        },
        second: 0
    },
    ne = {
        first: {
            leftTimes: 0,
            level: 0,
            fiat: !1,
            levelUpdated: !1,
            dailySpinTimes: 0,
            dailyWager: 0,
            levelWager: 200,
            vipSpinTimes: 0
        },
        second: 0
    },
    ae = {
        first: {
            endTime: 0,
            intervalTime: 60 * 6e4,
            lastBetUsd: "0",
            lastReceiveTime: 0,
            nextBetUsd: "0",
            nextReceiveTime: 0,
            receiveTimeMS: 0,
            receiveUsd: "0",
            rewards: [],
            sendBonus: !1,
            startTime: 0,
            status: 0,
            vipLevel: 0,
            viphostUserId: 0,
            tire: {
                currentTire: {
                    endWager: 49999,
                    index: 1,
                    startWager: 1e3,
                    tireName: "Basic",
                    vipRechargeRate: .18
                },
                tires: []
            }
        },
        second: 0
    },
    oe = {
        first: {
            currentTime: 0,
            rollTimes: 0
        },
        second: 0
    },
    ie = () => o.http.post("/activity/bonus-dashboard/get/"),
    re = () => o.http.post("/activity/bonus-dashboard/statistics/");

function Ae(s) {
    return o.http.post(`/activity/reward/receive/${s}/`)
}

function za() {
    const s = o.useLocalCurrency(),
        e = o.useSetting(),
        t = 1e3;
    return (a, n = !0, r = 2) => {
        const A = e.enableLocaleCurrency ? o.systemStore.getCurrencySymbol(e.localeCurrencyName) : "$",
            l = s.amount2local(new o.Decimal(a || 0)),
            c = l.gt(t) ? 0 : r,
            d = l.todp(c, o.Decimal.ROUND_DOWN).toNumber();
        return n ? d === 0 ? `${A}0.00` : `${A}${o.utils.numberWithCommas(e.localeCurrencyName,d)}` : d === 0 ? "0.00" : `${o.utils.numberWithCommas(e.localeCurrencyName,d)}`
    }
}

function ce(s, e, t) {
    const a = o.utils.serverTime().getTime();
    return t > 0 ? 2 : a >= e ? 3 : s > 0 ? 1 : 0
}

function N(s) {
    let e = 0,
        t = 0,
        a = 0;
    const n = [],
        r = [],
        A = [];
    if (s) {
        const l = s.first.freeSpins;
        l && l.length > 0 && l.map(m => {
            if (m.claimedTime === 0) {
                a++;
                const w = {
                    rewardId: m.rewardId,
                    rewardBonusId: m.rewardBonusId,
                    count: m.dailyNum
                };
                A.push(w)
            }
            return m
        });
        const c = s.first.cashbackBonus;
        c && c.length > 0 && c.map(m => (m.rewardId > 0 && ce(m.rewardId, m.expireTime, m.claimedTime) === 1 && (e++, n.push({
            count: m.count || 0,
            week: m.week || 0
        })), m));
        const d = s.first.lotteryBonus;
        d && d.length > 0 && d.map(m => {
            const f = m.currentLottery;
            if (f && f.rewardId > 0 && !(m.source === "newbie")) {
                t++;
                const b = {
                    rewardId: f.rewardId,
                    bonusAmount: m.totalCount
                };
                r.push(b)
            }
            return m
        })
    }
    return {
        deposit: e,
        lottery: t,
        freespin: a,
        deposits: n,
        lotterys: r,
        freespins: A
    }
}

function le(s) {
    let e = 0;
    const t = [];
    if (s && s.first) {
        const a = s.first;
        a && a.length > 0 && a.map(n => {
            n.claimedTime > 0 || (e++, t.push({
                rewardId: n.rewardId,
                rewardBonusId: n.rewardBonusId,
                currency: n.bonusCurrencyName,
                amount: n.bonusAmount
            }))
        })
    }
    return {
        lottery_winnings: e,
        lottery_winnings_list: t
    }
}

function ue(s) {
    if (!s || !s.second || s.second <= 0) return 0;
    const t = (s.first && s.first.rewards ? s.first.rewards : []).filter(a => a.rewardId > 0);
    return t ? t.length : 0
}
class j extends o.Howl {
    constructor(e) {
        super(g({
            preload: !1
        }, e))
    }
    play(e) {
        return this.state() === "unloaded" && this.load(), super.play(e)
    }
}
const de = "/modules/bonus/claim-d7b4098e.mp3";

function pe() {
    const s = new j({
        src: de
    });
    if (o.settingStore.state.soundEffectEnable) try {
        s.play()
    } catch (e) {}
}
const {
    useProxy: me,
    proxy: ge
} = o.valtio, fe = 24 * 60 * 60 * 1e3;
class he extends o.State {
    constructor(t) {
        super(t);
        B(this, "initData");
        B(this, "timer", null);
        B(this, "rollTimeDown", t => {
            t > 0 && (this.timer && clearInterval(this.timer), this.setState({
                timeDown: t
            }), this.state.timeDown = t, this.timer = setInterval(() => {
                const a = this.state.timeDown - 6e4;
                a <= 0 ? (this.state.showNotify = !0, this.setState({
                    timeDown: 0
                }), clearInterval(this.timer)) : this.setState({
                    timeDown: a
                })
            }, 1e3 * 60))
        });
        this.initData = this.initFn(), o.app.on("rewardClaim", () => {
            this.initFn()
        }), o.accountStore.waitLogin().then(() => {
            setTimeout(() => {
                this.firstInit()
            }, 1e3)
        })
    }
    async initFn() {
        if (await o.accountStore.initData, !o.accountStore.state.login) return o.accountStore.waitLogin().then(() => this.initFn()), this.state;
        try {
            this.setState({
                firstFlag: !1
            });
            const t = await Promise.all([ie(), re()]);
            if (t && t.length === 2) {
                let a = g({}, this.state);
                const n = t[0];
                let r = n.unclaimedCount || 0;
                if (n.totalBonusReceived && n.totalBonusReceived.first && (a.totalBonusReceived = n.totalBonusReceived), n.statistics && n.statistics.first && (a.statistics = n.statistics), n.depositBonus && n.depositBonus.first && (a.depositBonus = n.depositBonus), n.bonusUnlock && n.bonusUnlock.first && (a.bonusUnlock = n.bonusUnlock), n.quests && n.quests.first) {
                    a.quests = n.quests;
                    const A = n.quests.first.rewards;
                    if (A && A.length > 0) {
                        const l = A.filter(c => c.rewardId > 0);
                        l.length > 1 && (r = r + l.length - 1)
                    }
                    n.quests.second > 0 && o.accountStore.state.vipLevel >= Q && (r = Math.max(r - 1, 0))
                }
                n.spin && n.spin.first && (a.spin = n.spin, n.spin.second > 0 && o.accountStore.state.vipLevel >= Q && (r = Math.max(r - 1, 0))), n.rollCompetition && n.rollCompetition.first && (a.rollCompetition = n.rollCompetition, n.rollCompetition.second > 0 && (r = Math.max(r - 1, 0))), n.freeMoneyBonus && (a.freeMoneyBonus = n.freeMoneyBonus), n.lotteryWinningsBonus && (a.lotteryWinningsBonus = n.lotteryWinningsBonus), n.recharge && n.recharge.first && (a.recharge = n.recharge), n.monthlyBonus && n.monthlyBonus.first && (a.monthlyBonus = n.monthlyBonus), n.weeklyBonus && n.weeklyBonus.first && (a.weeklyBonus = n.weeklyBonus), n.sportsWeeklyBonus && n.sportsWeeklyBonus.first && (a.sportsWeeklyBonus = n.sportsWeeklyBonus), n.specialBonus && n.specialBonus.first && (a.specialBonus = this.sortRewardList(n.specialBonus)), a.unclaimedCount = r, a.refresh = !this.state.refresh, a.statisticsDetail = t[1], a.isLoading = !1, a.isError = !1;
                try {
                    this.setState(g({}, a))
                } catch (A) {}
            }
        } catch (t) {
            o.notify(t), this.setState({
                isError: !0
            })
        }
        return this.state
    }
    firstInit(t = !1) {
        (this.state.firstFlag || t) && this.initFn()
    }
    sortRewardList(t) {
        const a = g({}, t),
            n = [...t.first];
        return n.sort((r, A) => r.status - A.status), a.first = [...n], a
    }
    getGeneralBonusAmount() {
        const t = ue(this.state.quests),
            a = this.state.spin.second,
            n = this.state.rollCompetition.second,
            r = this.state.bonusUnlock ? this.state.bonusUnlock.second : 0,
            {
                deposit: A,
                lottery: l
            } = N(this.state.depositBonus),
            {
                lottery_winnings: c
            } = le(this.state.lotteryWinningsBonus);
        return t + a + n + r + A + l + c
    }
    getVipBonusAmount() {
        const t = this.state.recharge.second,
            a = this.state.weeklyBonus.second,
            n = this.state.monthlyBonus.second,
            r = this.state.sportsWeeklyBonus.second;
        return t + a + n + r
    }
    getFreeSpinBonusAmount() {
        const {
            freespin: t
        } = N(this.state.depositBonus);
        return t
    }
    async receiveReward(t) {
        if (!t) return;
        const a = [...this.state.specialBonus.first],
            n = a.findIndex(r => r.rewardId === t);
        if (n >= 0) {
            const r = g({}, a[n]);
            if (r && r.status === 0) try {
                await Ae(t), pe();
                const A = [...a];
                return A[n].status = 1, this.state.specialBonus = {
                    first: A,
                    second: this.subNumber(this.state.specialBonus.second)
                }, this.state.unclaimedCount = this.state.unclaimedCount - 1, !0
            } catch (A) {
                return o.notify(A), !1
            } else return !1
        } else return !1
    }
    subNumber(t) {
        return t <= 0 ? 0 : t - 1
    }
    setTimedownInfo(t) {
        let a = this.state.timeDown;
        if (t.spin && t.spin.second <= 0) {
            const n = this.state.nextSpinTime - Date.now();
            n <= a && (a = n, this.state.notifyType = "spin")
        }
        if (o.accountStore.state.vipLevel >= Z && t.recharge && t.recharge.second <= 0 && t.recharge.first.status >= 2) {
            const n = t.recharge.first.receiveTimeMS;
            n && n > 0 && n < a && (a = n, this.state.notifyType = "recharge")
        }
        a !== this.state.timeDown && this.rollTimeDown(a)
    }
    async updateLotteryFreeTicket(t) {
        this.state.isLotteryFreeTicketLoading || (this.state.isLotteryFreeTicketLoading = !0, o.http.get("/activity/recharge-bonus/lottery/").then(a => {
            let n = g({}, this.state.depositBonus);
            n.first.lotteryBonus = a, this.setState({
                isLotteryFreeTicketLoading: !1,
                depositBonus: n
            }), t && t(a)
        }).catch(a => {
            o.notify(a), this.setState({
                isLotteryFreeTicketLoading: !1
            })
        }))
    }
    async updateFreeSpin() {
        this.state.isFreeSpinLoading || (this.setState({
            isFreeSpinLoading: !0
        }), o.http.get("/activity/recharge-bonus/freeSpin/").then(t => {
            let a = g({}, this.state.depositBonus);
            a.first.freeSpins = t, this.setState({
                isFreeSpinLoading: !1,
                depositBonus: a
            })
        }).catch(t => {
            o.notify(t), this.setState({
                isFreeSpinLoading: !1
            })
        }))
    }
    async updateNewDeposit() {
        this.state.isNewDepositLoading || (this.setState({
            isNewDepositLoading: !0
        }), o.http.get("/activity/recharge-bonus/depositBonus/").then(t => {
            let a = g({}, this.state.depositBonus);
            a.first.cashbackBonus = t, this.setState({
                isNewDepositLoading: !1,
                depositBonus: a
            })
        }).catch(t => {
            o.notify(t), this.setState({
                isNewDepositLoading: !1
            })
        }))
    }
    async updateAll() {
        this.updateFreeSpin(), this.updateLotteryFreeTicket(), this.updateNewDeposit()
    }
    async updateFirstDeposit() {
        const t = this.state.depositBonus.first.cashbackBonus;
        t && (t == null ? void 0 : t.length) > 0 && this.updateAll()
    }
    async spinNextTime() {
        const t = o.utils.serverTime(),
            a = t.getUTCFullYear(),
            n = t.getUTCMonth(),
            r = t.getUTCDate() + 1;
        this.setState({
            nextSpinTime: Date.UTC(a, n, r, 0, 0, 0)
        })
    }
}
const v = new he(ge({
    refresh: !1,
    firstFlag: !0,
    unclaimedCount: 0,
    totalBonusReceived: {
        first: 0,
        second: 0
    },
    bonusUnlock: null,
    quests: g({}, se),
    spin: g({}, ne),
    depositBonus: g({}, _),
    nextSpinTime: Date.now(),
    recharge: g({}, ae),
    statistics: g({}, ee),
    statisticsDetail: g({}, $),
    rollCompetition: g({}, oe),
    weeklyBonus: g({}, z),
    monthlyBonus: g({}, z),
    sportsWeeklyBonus: g({}, te),
    freeMoneyBonus: null,
    lotteryWinningsBonus: {
        first: [],
        second: 0
    },
    specialBonus: {
        first: [],
        second: 0
    },
    timeDown: fe,
    showNotify: !1,
    notifyType: "",
    isLoading: !0,
    isError: !1,
    isFreeSpinLoading: !1,
    isLotteryFreeTicketLoading: !1,
    isNewDepositLoading: !1
}));

function Na() {
    return me(v.state)
}

function D(...s) {
    return s.filter(Boolean).join(" ")
}
be(s => Be(s).toString(36));

function be(s, e) {
    if (typeof s != "function" || e != null && typeof e != "function") throw new TypeError(FUNC_ERROR_TEXT);
    var t = function() {
        var a = arguments,
            n = e ? e.apply(this, a) : a[0],
            r = t.cache;
        if (r.has(n)) return r.get(n);
        var A = s.apply(this, a);
        return t.cache = r.set(n, A) || r, A
    };
    return t.cache = new Map, t
}

function Be(s, e) {
    var t, a, n, r, A, l, c, d;
    for (t = s.length & 3, a = s.length - t, n = e, A = 3432918353, l = 461845907, d = 0; d < a;) c = s.charCodeAt(d) & 255 | (s.charCodeAt(++d) & 255) << 8 | (s.charCodeAt(++d) & 255) << 16 | (s.charCodeAt(++d) & 255) << 24, ++d, c = (c & 65535) * A + (((c >>> 16) * A & 65535) << 16) & 4294967295, c = c << 15 | c >>> 17, c = (c & 65535) * l + (((c >>> 16) * l & 65535) << 16) & 4294967295, n ^= c, n = n << 13 | n >>> 19, r = (n & 65535) * 5 + (((n >>> 16) * 5 & 65535) << 16) & 4294967295, n = (r & 65535) + 27492 + (((r >>> 16) + 58964 & 65535) << 16);
    switch (c = 0, t) {
        case 3:
            c ^= (s.charCodeAt(d + 2) & 255) << 16;
        case 2:
            c ^= (s.charCodeAt(d + 1) & 255) << 8;
        case 1:
            c ^= s.charCodeAt(d) & 255, c = (c & 65535) * A + (((c >>> 16) * A & 65535) << 16) & 4294967295, c = c << 15 | c >>> 17, c = (c & 65535) * l + (((c >>> 16) * l & 65535) << 16) & 4294967295, n ^= c
    }
    return n ^= s.length, n ^= n >>> 16, n = (n & 65535) * 2246822507 + (((n >>> 16) * 2246822507 & 65535) << 16) & 4294967295, n ^= n >>> 13, n = (n & 65535) * 3266489909 + (((n >>> 16) * 3266489909 & 65535) << 16) & 4294967295, n ^= n >>> 16, n >>> 0
}
const De = "/modules/bonus/bonus-90a1735c.png",
    ye = "/modules/bonus/bonus_d-ed8d9039.png",
    we = "data:image/png;base64,",
    Ce = "/modules/bonus/bcdunlock-4c3e0f44.png",
    ve = "/modules/bonus/coin-968b6427.png",
    Se = "/modules/bonus/money-7fa4e398.png",
    Te = "/modules/bonus/rakeback-dee50f49.png",
    Ve = "/modules/bonus/spin-463f2a1f.png",
    ke = "/modules/bonus/spin_fiat-ca545efc.png",
    xe = "/modules/bonus/quests-7ae7831a.png",
    qe = "/modules/bonus/egg-4a882516.png",
    je = "/modules/bonus/roll-02855245.png",
    Ue = "/modules/bonus/recharge-e80e2af9.png",
    Ee = "/modules/bonus/weeklybonus-a15a7b47.png",
    Re = "/modules/bonus/monthlybonus-41ba9cad.png",
    Pe = "data:image/png;base64,",
    Qe = "data:image/png;base64,",
    ze = "data:image/png;base64,",
    Ne = "data:image/png;base64,",
    Me = "data:image/png;base64,",
    Ie = "/modules/bonus/contest-5e67a0ec.png",
    Le = "/modules/bonus/vipbonus-111c33a4.png",
    Oe = "/modules/bonus/levelup-40314c2d.png",
    Ye = "/modules/bonus/general-8f62a12e.png",
    Je = "/modules/bonus/sportsbonus-2d8f394d.png",
    Ge = "data:image/png;base64,",
    Fe = "data:image/png;base64,",
    We = "/modules/bonus/guide_header-f28d63d3.png",
    Ke = "data:image/png;base64,",
    He = "/modules/bonus/jackpot_ball_banner_s-03f82800.png",
    Xe = "/modules/bonus/jackpot_ball_banner_s_white-62ba2d53.png",
    Ze = "/modules/bonus/bonus_deposit-f4714a04.png",
    _e = "/modules/bonus/bonus_lottery-888e9bcf.png",
    $e = "data:image/png;base64,",
    C = {
        bonus_deposit: Ze,
        bonus_lottery: _e,
        bonus_spin: $e,
        bonus: De,
        bonus_d: ye,
        net: we,
        bcdunlock: Ce,
        coin: ve,
        money: Se,
        rakeback: Te,
        spin: Ve,
        spin_fiat: ke,
        quests: xe,
        roll: je,
        egg: qe,
        recharge: Ue,
        weeklybonus: Ee,
        monthlybonus: Re,
        spin_bg: ze,
        turntable: Qe,
        pointer: Pe,
        imgprogress: Me,
        recharge_s: Ne,
        vipbonus: Le,
        levelup: Oe,
        contest: Ie,
        general: Ye,
        sportsbonus: Je,
        ballWhite: Ge,
        ball: Fe,
        guideHeader: We,
        jackpotBall: Ke,
        jackpotBallBanner: He,
        jackpotBallBannerWhite: Xe
    };

function et(s) {
    const e = {
        expiredTime: 0,
        fromCurrencyAmount: String(s),
        fromCurrencyName: "BCL",
        lotteryList: [{
            numbers: [],
            jackpotBallNumber: 0,
            quantity: 1,
            pickType: 1
        }],
        toCurrencyName: "BCL"
    };
    return o.http.post("/lottery/five-plus-one/buy/", e)
}
const V = function({
    nums: e,
    jackpotNum: t,
    className: a
}) {
    const n = o.useIsDarken();
    return i.jsxs("div", {
        className: D(at, a),
        children: [i.jsx("img", {
            className: "img-bg",
            src: n ? C.jackpotBallBanner : C.jackpotBallBannerWhite,
            alt: ""
        }), i.jsxs("div", {
            className: "jackpot-wrap",
            children: [i.jsx("div", {
                className: "nums",
                children: e.map(r => i.jsxs("div", {
                    className: "ball",
                    children: [i.jsx("img", {
                        src: n ? C.ball : C.ballWhite,
                        alt: ""
                    }), i.jsx("div", {
                        className: "num",
                        children: r
                    })]
                }, r))
            }), i.jsx("div", {
                className: "jackpotNum",
                children: i.jsxs("div", {
                    className: "ball",
                    children: [i.jsx("img", {
                        src: C.jackpotBall
                    }), i.jsx("div", {
                        className: "num",
                        children: t
                    })]
                })
            })]
        })]
    })
};

function k(s) {
    return s > 9 ? s : "0" + s
}

function tt({
    number: s
}) {
    const {
        t: e
    } = o.useTranslation(), t = o.usePop(), a = o.useIsDarken(), n = o.useNavigate(), {
        data: r
    } = o.useAsync(() => et(Number(s)));
    return r ? i.jsxs("div", {
        className: nt,
        children: [i.jsxs("div", {
            className: "header",
            children: [i.jsx("img", {
                src: C.guideHeader,
                className: ""
            }), i.jsx("div", {
                className: "big-title",
                children: e("Lottery Jackpot")
            })]
        }), i.jsxs("div", {
            className: "scroll-wrapper",
            children: [i.jsx(o.CloseIcon, {
                onClick: () => t.close()
            }), i.jsxs("div", {
                className: "next-time",
                children: [i.jsx("div", {
                    className: "txt",
                    children: e("Next Draw Start in")
                }), i.jsx("div", {
                    className: D("time", a ? "dark" : ""),
                    children: i.jsx(o.Countdown, {
                        endTime: r.expireTimeInMs + Date.now(),
                        onEnd: () => {},
                        children: ({
                            hours: A,
                            minutes: l,
                            seconds: c
                        }) => i.jsxs(i.Fragment, {
                            children: [i.jsxs("span", {
                                children: [k(A), "h"]
                            }), ":", i.jsxs("span", {
                                children: [k(l), "m"]
                            }), ":", i.jsxs("span", {
                                children: [k(c), "s"]
                            })]
                        })
                    })
                })]
            }), i.jsx("div", {
                className: D("spectial-text", a ? "dark" : ""),
                dangerouslySetInnerHTML: {
                    __html: Number(s) > 1 ? `${e("YOU GOT")} <span class="green">${Number(s)} ${e("Ticket Numbers")}</span>` : e("YOUR <span class='green'>Ticket NUMBERS</span> ARE..")
                }
            }), i.jsx("div", {
                className: "jackpotball-wrapper",
                children: Number(s) > 1 ? i.jsxs(h.Fragment, {
                    children: [i.jsx(V, {
                        nums: r.numbers,
                        jackpotNum: r.jackpotBallNumber
                    }), i.jsx(V, {
                        nums: r.numbers,
                        jackpotNum: r.jackpotBallNumber,
                        className: "float-jackpot"
                    })]
                }) : i.jsx(V, {
                    nums: r.numbers,
                    jackpotNum: r.jackpotBallNumber
                })
            }), i.jsx("div", {
                className: "btn-wrapper",
                children: i.jsx(o.Button, {
                    type: "conic4",
                    className: "btn",
                    onClick: () => {
                        n("/lottery/myBets"), t.close()
                    },
                    children: e("Go to Lottery")
                })
            })]
        })]
    }) : i.jsx(o.Loading, {})
}
const st = X(tt),
    nt = "v1ajtup7",
    at = "jbter41";
var Ma = globalThis["framer-motion"];
const ot = "/modules/bonus/glod_bg-ff13c349.png",
    it = "/modules/bonus/bg1-36f9d669.png";

function rt({
    time: s
}) {
    const [e, t] = o.useSetState({
        time: s
    });
    return h.useEffect(() => {
        const a = e.time - 1,
            n = setTimeout(() => {
                t({
                    time: a
                })
            }, 1e3);
        return () => clearTimeout(n)
    }, [e.time]), e.time <= 0 ? null : i.jsxs("div", {
        className: "time",
        children: [e.time, "s"]
    })
}

function At({
    time: s,
    amount: e,
    currency: t
}) {
    const a = o.usePop(),
        [n, r] = o.useSetState({
            rotate: !1
        }),
        {
            t: A
        } = o.useTranslation();
    return h.useEffect(() => {
        const l = setTimeout(() => r({
                rotate: !0
            }), 1300),
            c = setTimeout(() => a.close(), s * 1e3);
        return () => {
            clearTimeout(l), clearTimeout(c)
        }
    }, []), i.jsxs("div", {
        className: ct,
        children: [i.jsx(rt, {
            time: s
        }), i.jsx("img", {
            className: D("img-bg", n.rotate && "rotate"),
            src: it,
            alt: ""
        }), i.jsx("img", {
            className: "img_glod",
            src: ot,
            alt: ""
        }), i.jsxs("div", {
            className: "content",
            children: [i.jsx("div", {
                className: "txt tit ttu",
                children: A("Rewards Claimed")
            }), i.jsxs("div", {
                className: "amount",
                children: [e, " ", o.systemStore.getAlias(t)]
            }), i.jsx("div", {
                className: "txt ttu",
                children: A("Bonus")
            })]
        })]
    })
}
const ct = "s1czd39q",
    lt = "/modules/bonus/bg3-135f1264.png";

function ut({
    data: s,
    total: e
}) {
    const [t, a] = o.useSetState({
        rotate: !1,
        chooseIndex: 0
    }), n = o.useNavigate(), {
        t: r
    } = o.useTranslation(), A = o.usePop(), l = o.useDialog();
    h.useEffect(() => {
        const m = setTimeout(() => a({
            rotate: !0
        }), 1300);
        return () => {
            clearTimeout(m)
        }
    }, []);
    const c = () => {
            l.close(), A.close(), n(`/game/${s.gameUnique}?fsId=${s.fsId}&fs=${s.fsRoundTotal-s.fsRoundUsed}`)
        },
        d = Math.ceil(e / s.dailyNum);
    return i.jsxs("div", {
        className: dt,
        children: [i.jsx(o.CloseIcon, {
            onClick: () => A.close()
        }), i.jsx("img", {
            className: D("img-bg", t.rotate && "rotate"),
            src: lt,
            alt: ""
        }), i.jsxs("div", {
            className: "content",
            children: [i.jsx("div", {
                className: "txt tit",
                children: r("{{dailyNum}} Free Spin Claimed", {
                    dailyNum: String(s.dailyNum)
                })
            }), i.jsx("div", {
                className: "txt sub-tit",
                children: r("You can claim {{dailyNum}} FS per day for {{days}} days in a row in total {{total}} free spins.", {
                    dailyNum: String(s.dailyNum),
                    days: d,
                    total: e
                })
            }), i.jsxs("div", {
                className: "games",
                children: [i.jsx("img", {
                    src: s.thumbnail,
                    alt: ""
                }), i.jsx("div", {
                    className: "provider",
                    children: s.providerName
                })]
            }), i.jsx(o.Button, {
                onClick: c,
                type: "conic",
                children: r("Play")
            })]
        })]
    })
}
const dt = "s82nv2u",
    pt = "/modules/bonus/ticket_bg-a8bf0d1c.png",
    mt = "/modules/bonus/bg2-2d386db4.png",
    gt = "/modules/bonus/ball-3117206e.png",
    ft = "/modules/bonus/ball_w-e380db03.png";

function ht({
    ball: s,
    jackpot: e
}) {
    const t = o.useIsDarken();
    return i.jsxs("div", {
        className: D("ball-wrap", Dt),
        children: [i.jsx("img", {
            className: "ball-bg",
            src: t ? gt : ft,
            alt: ""
        }), i.jsxs("div", {
            className: "ball-list",
            children: [s.map((a, n) => i.jsx("div", {
                className: "ball",
                children: a
            }, n)), i.jsx("div", {
                className: "jackpot",
                children: e
            })]
        })]
    })
}

function bt({
    time: s
}) {
    const [e, t] = o.useSetState({
        time: s
    });
    return h.useEffect(() => {
        const a = e.time - 1,
            n = setTimeout(() => {
                t({
                    time: a
                })
            }, 1e3);
        return () => clearTimeout(n)
    }, [e.time]), e.time <= 0 ? null : i.jsxs("div", {
        className: "time",
        children: [e.time, "s"]
    })
}

function Bt({
    time: s,
    data: e
}) {
    const [t, a] = o.useSetState({
        rotate: !1
    }), n = o.usePop(), {
        t: r
    } = o.useTranslation();
    h.useEffect(() => {
        const l = setTimeout(() => a({
                rotate: !0
            }), 1300),
            c = setTimeout(() => n.close(), s * 1e3);
        return () => {
            clearTimeout(l), clearTimeout(c)
        }
    }, []);
    const A = e && e.currentLottery;
    return A ? i.jsxs("div", {
        className: M,
        children: [i.jsx(bt, {
            time: s
        }), i.jsx("img", {
            className: D("img-bg", t.rotate && "rotate"),
            src: mt,
            alt: ""
        }), i.jsx("img", {
            className: "img_glod",
            src: pt,
            alt: ""
        }), i.jsxs("div", {
            className: "content",
            children: [i.jsxs("div", {
                className: "txt tit ttu",
                children: [r("Rewards Claimed"), "!"]
            }), i.jsx(ht, {
                ball: A.lotteryResult.numbers,
                jackpot: A.lotteryResult.jackpotBallNumber
            }), i.jsx("div", {
                className: "txt ttu",
                children: r("Bonus")
            })]
        })]
    }) : i.jsx("div", {
        className: M,
        children: i.jsx(o.Empty, {})
    })
}
const M = "s1w0s9pt",
    Dt = "b13nhtvt",
    x = "data:image/png;base64,";

function yt(s) {
    return v.state.depositBonus.first.cashbackBonus.find(a => a.count === s)
}

function wt(s) {
    return o.http.post("/activity/taskhub/takeReward/", {
        rewardId: s
    })
}
const q = Y.memo(function({
    tit: e
}) {
    const [t] = h.useState(e);
    return i.jsx("div", {
        className: "bonus-tit",
        children: t
    })
});

function Ia({
    count: s
}) {
    const e = yt(s),
        t = 3,
        a = o.useSetting(),
        {
            t: n
        } = o.useTranslation(),
        r = o.useDialog(),
        A = o.usePop(),
        [l, c] = o.useSetState({
            claimAll: !1,
            loading: !1,
            claimCash: !1,
            claimSpin: !1,
            hasSpin: e.freeSpin > 0,
            claimLottery: !1,
            hasLottery: e.lottery > 0
        }),
        d = o.useNavigate(),
        m = h.useCallback(() => {
            A.push(i.jsx(At, {
                time: t,
                amount: e.bonusAmount,
                currency: e.bonusCurrencyName
            }))
        }, []),
        f = h.useCallback(y => {
            A.push(i.jsx(Bt, {
                data: y,
                time: t
            }))
        }, []),
        w = h.useCallback(y => {
            A.push(i.jsx(ut, {
                data: y,
                total: e.freeSpin
            }), {
                closeable: !1
            })
        }, []);
    async function b() {
        if (l.claimAll) r.close(), d("/bonus");
        else {
            c({
                loading: !0
            });
            const y = await wt(e.rewardId);
            m(), c({
                claimCash: !0
            }), (y.lottery || y.freeSpinBean) && await o.utils.delay((t + .5) * 1e3), y.lottery && (f(y.lottery), c({
                claimLottery: !0
            }), y.freeSpinBean && await o.utils.delay((t + .5) * 1e3)), l.hasSpin && y.freeSpinBean && (w(y.freeSpinBean), c({
                claimSpin: !0
            })), v.updateAll(), c({
                loading: !1,
                claimAll: !0
            })
        }
    }
    const S = a.enableLocaleCurrency;
    return e ? i.jsx(o.Dialog, {
        className: Ct,
        children: i.jsxs(o.ScrollView, {
            children: [i.jsx("div", {
                className: "title",
                children: n("Congratulations!")
            }), i.jsx("div", {
                className: "sub-tit",
                children: n("Claim your Deposit Bonus below!")
            }), i.jsxs("div", {
                className: D("bonus-item bg-0", (l.claimAll || l.claimCash) && "claimed"),
                children: [i.jsx(q, {
                    tit: `${o.utils.numberWithCommas(S&&a.localeCurrencyName,e.bonusAmount)} ${o.systemStore.getAlias(e.bonusCurrencyName)} ${n("Bonus")}`
                }), i.jsxs("div", {
                    className: "claim-desc",
                    children: [i.jsx("img", {
                        src: x
                    }), " ", n("Claimed")]
                }), i.jsx("div", {
                    className: "bonus-desc",
                    dangerouslySetInnerHTML: {
                        __html: n("Deposit more to get more bonuses.")
                    }
                })]
            }), l.hasLottery && i.jsxs("div", {
                className: D("bonus-item bg-1", (l.claimLottery || l.claimAll) && "claimed"),
                children: [i.jsx(q, {
                    tit: `${e.lottery} ${n("Lottery Tickets")}`
                }), i.jsxs("div", {
                    className: "claim-desc",
                    children: [i.jsx("img", {
                        src: x
                    }), " ", n("Claimed")]
                }), i.jsx("div", {
                    className: "bonus-desc",
                    dangerouslySetInnerHTML: {
                        __html: n("You can only redeem <b>1 ticket per day.</b>")
                    }
                })]
            }), l.hasSpin && i.jsxs("div", {
                className: D("bonus-item bg-2", (l.claimSpin || l.claimAll) && "claimed"),
                children: [i.jsx(q, {
                    tit: `${e.freeSpin} ${n("Free Spin")}`
                }), i.jsxs("div", {
                    className: "claim-desc",
                    children: [i.jsx("img", {
                        src: x
                    }), " ", n("Claimed")]
                }), i.jsx("div", {
                    className: "bonus-desc",
                    dangerouslySetInnerHTML: {
                        __html: n("You can only claim <b>20 free spin per day for 5 days.</b>")
                    }
                })]
            }), i.jsx(o.Button, {
                type: "conic",
                onClick: b,
                children: l.claimAll ? n("Check in My Bonus") : n("Claim All")
            })]
        })
    }) : i.jsx(o.Dialog, {
        children: i.jsx(o.Empty, {})
    })
}
const Ct = "s1em6k3k";

function La() {
    const {
        t: s
    } = o.useTranslation();
    return function(e) {
        return s(e === 1 ? "1st Deposit Bonus" : e === 2 ? "2nd Deposit Bonus" : e === 3 ? "3rd Deposit Bonus" : e === 4 ? "4th Deposit Bonus" : "Deposit Bonus")
    }
}

function Oa() {
    const {
        t: s
    } = o.useTranslation();
    return function(e) {
        return e === 1 ? s("Monday") : e === 2 ? s("Tuesday") : e === 3 ? s("Wednesday") : e === 4 ? s("Thursday") : e === 5 ? s("Friday") : e === 6 ? s("Saturday") : e === 7 ? s("Sunday") : ""
    }
}
const vt = "/modules/bonus/disabled-872efa0c.svg",
    St = "/modules/bonus/recharge-586261e1.json",
    Tt = "/modules/bonus/rechargew-white-fe1cd6d5.json",
    Vt = "/modules/bonus/recharge_reward-6fe75678.json",
    kt = "/modules/bonus/recharge-0-86e44653.png",
    xt = "/modules/bonus/recharge-0-white-bba84859.png",
    qt = "/modules/bonus/recharge-25-00a320b4.png",
    jt = "/modules/bonus/recharge-25-white-95bcca03.png",
    Ut = "/modules/bonus/recharge-50-e17fcb0e.png",
    Et = "/modules/bonus/recharge-50-white-707cf11a.png",
    Rt = "/modules/bonus/recharge-75-65f4c194.png",
    Pt = "/modules/bonus/recharge-75-white-f019d73c.png",
    Qt = "/modules/bonus/recharge-100-22090dcd.png",
    zt = "/modules/bonus/recharge-100-white-6da3ced4.png";

function Ya() {
    const s = o.useIsDarken();
    return e => e < 25 ? s ? kt : xt : e < 50 ? s ? qt : jt : e < 75 ? s ? Ut : Et : e < 100 ? s ? Rt : Pt : s ? Qt : zt
}
const Nt = {
    disabled: vt,
    rechargeLottile: St,
    rechargeLottileWhite: Tt,
    rechargeReward: Vt
};
var I = globalThis["@bc/pixi"];
const Mt = {
        list: [{
            value: 0,
            time: 0
        }, {
            value: 1,
            time: .5
        }, {
            value: 0,
            time: 1
        }],
        isStepped: !1
    },
    It = {
        list: [{
            value: .05,
            time: 0
        }, {
            value: .3,
            time: 1
        }],
        isStepped: !1,
        minimumScaleMultiplier: 2
    },
    Lt = {
        start: "#3c5653",
        end: "#6dcaa4"
    },
    Ot = {
        start: 10,
        end: 20,
        minimumSpeedMultiplier: 1.3
    },
    Yt = {
        x: 0,
        y: 0
    },
    Jt = 0,
    Gt = {
        min: 0,
        max: 260
    },
    Ft = !1,
    Wt = {
        min: 0,
        max: 0
    },
    Kt = {
        min: 8,
        max: 12
    },
    Ht = "normal",
    Xt = .5,
    Zt = -1,
    _t = 500,
    $t = {
        x: 0,
        y: 0
    },
    es = !1,
    ts = "rect",
    ss = {
        x: 0,
        y: 0,
        w: 464,
        h: 300
    },
    ns = !0,
    as = !0,
    os = {
        alpha: Mt,
        scale: It,
        color: Lt,
        speed: Ot,
        acceleration: Yt,
        maxSpeed: Jt,
        startRotation: Gt,
        noRotation: Ft,
        rotationSpeed: Wt,
        lifetime: Kt,
        blendMode: Ht,
        frequency: Xt,
        emitterLifetime: Zt,
        maxParticles: _t,
        pos: $t,
        addAtBack: es,
        spawnType: ts,
        spawnRect: ss,
        emit: ns,
        autoUpdate: as
    },
    is = "data:image/png;base64,";
const rs = Y.memo(function({
        config: e = {}
    }) {
        const t = Object.assign(JSON.parse(JSON.stringify(os)), e);
        return i.jsx("div", {
            className: `${As} particle-layer`,
            children: i.jsx(I.PixiApplication, {
                width: t.spawnRect.w,
                height: t.spawnRect.h,
                fps: 30,
                children: i.jsx(I.Particles, {
                    textures: is,
                    config: t
                })
            })
        })
    }),
    As = "p18cf4a2";
const cs = {
    scale: {
        list: [{
            value: 1,
            time: 0
        }, {
            value: 1.5,
            time: 1
        }]
    },
    color: {
        start: "#892aff",
        end: "#892aff"
    },
    maxParticles: 8,
    spawnRect: {
        w: 414,
        h: 580
    }
};

function Ja({
    amount: s,
    currencyName: e
}) {
    const {
        t
    } = o.useTranslation(), a = o.usePop(), n = h.useRef(null);
    return h.useEffect(() => {
        setTimeout(() => {
            var r;
            (r = n.current) == null || r.play(0)
        }, 300)
    }, []), i.jsxs("div", {
        className: `result-pop ${ls}`,
        children: [i.jsx(o.Lottie, {
            className: "lottie-wrap",
            ref: n,
            path: Nt.rechargeReward
        }), i.jsx(rs, {
            config: cs
        }), i.jsxs("div", {
            className: "front",
            children: [i.jsx("div", {
                className: "recharge",
                children: t("Recharge")
            }), i.jsx("div", {
                className: "text",
                children: t("Congrats! You got")
            }), i.jsxs("div", {
                className: "result-value flex-center",
                children: [i.jsx("div", {
                    className: "amount",
                    children: s
                }), i.jsx("div", {
                    className: "currency-name",
                    children: o.systemStore.getAlias(e)
                })]
            }), i.jsx(o.Button, {
                onClick: () => a.close(),
                type: "conic",
                children: t("Confirm")
            })]
        })]
    })
}
const ls = "r1rt371y";
var U = globalThis["lodash-es"];
const Ga = function({
    rewardItem: e,
    hideContent: t = !1
}) {
    const {
        t: a
    } = o.useTranslation(), n = o.useSystem(), r = o.usePop(), A = e.rewards || [], l = n.rewardTypes[e.rewardType] || {}, c = A.slice(0, 1), d = e.status !== 0, m = e.rewardType === "vip_give_bonus" ? C.levelup : l.bgSmall || C.general, f = async () => {
        if (!d && (await v.receiveReward(e.rewardId), e.rewardType === "depositbcl")) {
            let b = "0";
            e.rewards && e.rewards[0] && (b = e.rewards[0].amount), b && r.push(i.jsx(st, {
                number: e.rewards[0].amount
            }))
        }
    }, w = t ? l.name : e.title;
    return i.jsxs("div", {
        className: D("reward-item", d && "claimed-item"),
        children: [i.jsx("div", {
            className: "reward-img-wrap",
            children: i.jsx("img", {
                alt: "reward",
                src: m
            })
        }), i.jsxs("div", {
            className: "reward-text",
            children: [i.jsx("p", {
                className: "title",
                children: w || ""
            }), !t && i.jsx("p", {
                className: "desc",
                children: e.content || ""
            }), i.jsx("div", {
                className: "coin-list",
                children: c.map((b, S) => i.jsx(o.BC.CoinFormat, {
                    name: b.name,
                    amount: Number(b.amount),
                    icon: !0,
                    sign: !0,
                    showName: !0
                }, b.name + S))
            })]
        }), i.jsx(o.Button, {
            onClick: f,
            style: {
                opacity: d ? .5 : 1
            },
            type: d ? "gray" : "conic",
            disabled: d,
            children: a(d ? "Claimed" : "Claim")
        })]
    })
};
var us = globalThis.gsap;
const ds = O(us),
    ps = "/modules/bonus/banner_bronze-f02bfb81.png",
    ms = "/modules/bonus/banner_silver-b64ba422.png",
    gs = "/modules/bonus/banner_gold-55ef522f.png",
    fs = "/modules/bonus/banner_platinum-db3a977d.png",
    hs = "/modules/bonus/banner_diamond-7d4938f7.png",
    bs = "/modules/bonus/point_bronze-0d249168.png",
    Bs = "/modules/bonus/point_silver-e07c5b71.png",
    Ds = "/modules/bonus/point_gold-ef89e212.png",
    ys = "/modules/bonus/point_diamond-3f948ba0.png",
    ws = "/modules/bonus/point_platinum-1945da83.png",
    Cs = "/modules/bonus/btn_luckspin-c9fdbf75.png",
    vs = "/modules/bonus/btn_megaspin-13327da9.png",
    Ss = "/modules/bonus/btn_superspin-642fce40.png",
    Ts = "/modules/bonus/tbtn_luckspin_1-15a52ea4.png",
    Vs = "/modules/bonus/tbtn_luckspin_2-90fbcfc3.png",
    ks = "/modules/bonus/tbtn_megaspin_1-76eb5830.png",
    xs = "/modules/bonus/tbtn_megaspin_2-1147ec4e.png",
    qs = "/modules/bonus/tbtn_superspin_1-0bcfa4f1.png",
    js = "/modules/bonus/tbtn_superspin_2-6176f80f.png",
    Us = "/modules/bonus/spin_bronze-4dd81880.png",
    Es = "/modules/bonus/spin_silver-5f9d2617.png",
    Rs = "/modules/bonus/spin_gold-3002c02a.png",
    Ps = "/modules/bonus/spin_platinum-d3b74f52.png",
    Qs = "/modules/bonus/spin_diamond-75e21109.png",
    zs = "data:image/png;base64,",
    Ns = "/modules/bonus/lucky_lottery-d21ba9a9.png",
    Ms = "/modules/bonus/mega_lottery-62df5a22.png",
    Is = "/modules/bonus/super_lottery-89c9893f.png",
    Ls = "/modules/bonus/lottery-1f600a06.png",
    Os = "/modules/bonus/spin_center1-47d8700e.png",
    Ys = "/modules/bonus/spin_center2-ed096c8d.png",
    Js = "/modules/bonus/spin_center3-deabf1f7.png",
    Gs = "/modules/bonus/spin_center4-bf82f1fd.png",
    Fs = "/modules/bonus/spin_center5-0f4ed38a.png",
    Ws = "/modules/bonus/banner1-576cb7b8.png",
    Ks = "/modules/bonus/banner2-3eec290e.png",
    Hs = "/modules/bonus/banner3-2e37b012.png",
    Xs = "/modules/bonus/banner4-2a032d56.png",
    Zs = "/modules/bonus/banner5-a1776e9e.png",
    _s = "/modules/bonus/br_banner1-b991aa34.png",
    $s = "/modules/bonus/br_banner2-4ee711ee.png",
    en = "/modules/bonus/br_banner3-27c5d5d8.png",
    tn = "/modules/bonus/br_banner4-89a06096.png",
    sn = "/modules/bonus/br_banner5-79cde9fe.png",
    nn = "/modules/bonus/bg1-a9f0f52e.png",
    an = "/modules/bonus/bg2-97e66dc1.png",
    on = "/modules/bonus/bg3-90616c54.png",
    rn = "/modules/bonus/bg4-c2a7d8f6.png",
    An = "/modules/bonus/gold-57a1d52f.png",
    cn = "/modules/bonus/light-f1abdd91.png",
    ln = "/modules/bonus/tag1-569f449b.png",
    un = "/modules/bonus/tag2-610935df.png",
    dn = "/modules/bonus/tag3-becb51f6.png",
    pn = "/modules/bonus/slots-69cd2ade.png",
    mn = "data:image/png;base64,",
    gn = "data:image/png;base64,",
    fn = "data:image/png;base64,",
    hn = "data:image/png;base64,",
    bn = "data:image/png;base64,",
    Bn = "data:image/png;base64,",
    Dn = "/modules/bonus/light1-fb8f40d9.png",
    yn = "/modules/bonus/light2-377ecfaa.png",
    wn = "/modules/bonus/light3-935bc13f.png",
    Cn = "/modules/bonus/light4-7876cd02.png",
    vn = "/modules/bonus/light5-76fd95a2.png",
    Sn = "data:image/png;base64,",
    Tn = "data:image/png;base64,",
    Vn = "data:image/png;base64,",
    kn = "/modules/bonus/spinLight-2993efaa.png",
    xn = "data:image/png;base64,",
    qn = "data:image/png;base64,",
    jn = {
        list: [{
            value: 0,
            time: 0
        }, {
            value: 1,
            time: .5
        }, {
            value: 0,
            time: 1
        }],
        isStepped: !1
    },
    Un = {
        start: .1,
        end: .3,
        minimumScaleMultiplier: 2
    },
    En = {
        start: "#ffffff",
        end: "#ffffff"
    },
    Rn = {
        start: 15,
        end: 10,
        minimumSpeedMultiplier: 3
    },
    Pn = {
        x: 0,
        y: 0
    },
    Qn = 0,
    zn = {
        min: 0,
        max: 360
    },
    Nn = !1,
    Mn = {
        min: 0,
        max: 0
    },
    In = {
        min: 2,
        max: 5
    },
    Ln = "normal",
    On = .1,
    Yn = -1,
    Jn = 100,
    Gn = {
        x: 0,
        y: 0
    },
    Fn = !1,
    Wn = "circle",
    Kn = {
        x: 50,
        y: 50,
        r: 2
    },
    Hn = {
        alpha: jn,
        scale: Un,
        color: En,
        speed: Rn,
        acceleration: Pn,
        maxSpeed: Qn,
        startRotation: zn,
        noRotation: Nn,
        rotationSpeed: Mn,
        lifetime: In,
        blendMode: Ln,
        frequency: On,
        emitterLifetime: Yn,
        maxParticles: Jn,
        pos: Gn,
        addAtBack: Fn,
        spawnType: Wn,
        spawnCircle: Kn
    },
    Xn = "/modules/bonus/japan_background-2ec23eb2.png",
    Zn = "/modules/bonus/japan_banner_bronze-eb299c23.png",
    _n = "/modules/bonus/japan_banner_silver-a189579a.png",
    $n = "/modules/bonus/japan_banner_gold-d8e32188.png",
    ea = "/modules/bonus/japan_banner_platinum-4ab0f25c.png",
    ta = "/modules/bonus/japan_banner_diamond-c142751c.png",
    sa = "/modules/bonus/japan_spin-91cdf35b.png",
    na = "/modules/bonus/japan_spin_center-e17caa98.png",
    aa = "/modules/bonus/japan_point-2414c1ef.png",
    oa = "/modules/bonus/japan_spin_light-bc2612b5.png",
    ia = "/modules/bonus/japan_tbtn_luckspin_1-8cf7c422.png",
    ra = "/modules/bonus/japan_tbtn_megaspin_1-9a262ef0.png",
    Aa = "/modules/bonus/japan_tbtn_superspin_1-0486ea34.png",
    u = {
        banner_bronze: ps,
        banner_silver: ms,
        banner_gold: gs,
        banner_platinum: fs,
        banner_diamond: hs,
        point_bronze: bs,
        point_silver: Bs,
        point_gold: Ds,
        point_diamond: ys,
        point_platinum: ws,
        btn_luckspin: Cs,
        btn_megaspin: vs,
        btn_superspin: Ss,
        tbtn_luckspin_1: Ts,
        tbtn_luckspin_2: Vs,
        tbtn_megaspin_1: ks,
        tbtn_megaspin_2: xs,
        tbtn_superspin_1: qs,
        tbtn_superspin_2: js,
        spin_bronze: Us,
        spin_silver: Es,
        spin_gold: Rs,
        spin_platinum: Ps,
        spin_diamond: Qs,
        tag: Sn,
        crown: Tn,
        laurel: Vn,
        bright: qn,
        bright_emitter: Hn,
        bg_lottery: Ls,
        spin_center1: Os,
        spin_center2: Ys,
        spin_center3: Js,
        spin_center4: Gs,
        spin_center5: Fs,
        spin_diamond_fuck: zs,
        star: xn,
        spinLight3: kn,
        fiat_banner1: Ws,
        fiat_banner2: Ks,
        fiat_banner3: Hs,
        fiat_banner4: Xs,
        fiat_banner5: Zs,
        fiat_banner1_br: _s,
        fiat_banner2_br: $s,
        fiat_banner3_br: en,
        fiat_banner4_br: tn,
        fiat_banner5_br: sn,
        fiat_gold: An,
        fiat_light: cn,
        fiat_tag1: ln,
        fiat_tag2: un,
        fiat_tag3: dn,
        fiat_bg1: nn,
        fiat_bg2: an,
        fiat_bg3: on,
        fiat_bg4: rn,
        fiat_btn1: mn,
        fiat_btn2: gn,
        fiat_btn3: fn,
        fiat_btn1_active: hn,
        fiat_btn2_active: bn,
        fiat_btn3_active: Bn,
        fiat_slot: pn,
        fiat_slotlight1: Dn,
        fiat_slotlight2: yn,
        fiat_slotlight3: wn,
        fiat_slotlight4: Cn,
        fiat_slotlight5: vn,
        japanBackground: Xn,
        japanBannerBronze: Zn,
        japanBannerSliver: _n,
        japanBannerGold: $n,
        japanBannerPlatinum: ea,
        japanBannerDiamond: ta,
        japanSpin: sa,
        japanSpinCenter: na,
        japanPoint: aa,
        japanSpanLight: oa,
        japanTbtnLuckSpin1: ia,
        japanTbtnMegaSpin1: ra,
        japanTbtnSuperSpin1: Aa
    };

function Fa(s) {
    switch (s) {
        case 2:
            return u.fiat_banner2;
        case 3:
            return u.fiat_banner3;
        case 4:
            return u.fiat_banner4;
        case 5:
            return u.fiat_banner5;
        default:
            return u.fiat_banner1
    }
}

function Wa(s) {
    switch (s) {
        case 2:
            return u.fiat_banner2_br;
        case 3:
            return u.fiat_banner3_br;
        case 4:
            return u.fiat_banner4_br;
        case 5:
            return u.fiat_banner5_br;
        default:
            return u.fiat_banner1_br
    }
}

function Ka(s) {
    switch (s) {
        case 0:
            return Ns;
        case 1:
            return Is;
        case 2:
            return Ms
    }
}

function ca(s) {
    switch (s) {
        case 2:
            return u.spin_silver;
        case 3:
            return u.spin_gold;
        case 4:
            return u.spin_platinum;
        case 5:
            return u.spin_diamond;
        default:
            return u.spin_bronze
    }
}

function Ha(s) {
    switch (s) {
        default: return u.japanSpin
    }
}

function Xa(s) {
    switch (s) {
        case 2:
            return u.spin_center2;
        case 3:
            return u.spin_center3;
        case 4:
            return u.spin_center4;
        case 5:
            return u.spin_center5;
        default:
            return u.spin_center1
    }
}

function Za(s) {
    switch (s) {
        default: return u.japanSpinCenter
    }
}

function _a(s) {
    switch (s) {
        case 2:
            return u.point_silver;
        case 3:
            return u.point_gold;
        case 4:
            return u.point_platinum;
        case 5:
            return u.point_diamond;
        default:
            return u.point_bronze
    }
}

function $a(s) {
    switch (s) {
        default: return u.japanPoint
    }
}

function eo(s) {
    switch (s) {
        case 1:
            return u.btn_superspin;
        case 2:
            return u.btn_megaspin;
        default:
            return u.btn_luckspin
    }
}

function to(s) {
    switch (s) {
        case 2:
            return u.banner_silver;
        case 3:
            return u.banner_gold;
        case 4:
            return u.banner_platinum;
        case 5:
            return u.banner_diamond;
        default:
            return u.banner_bronze
    }
}

function so(s) {
    switch (s) {
        case 2:
            return u.japanBannerSliver;
        case 3:
            return u.japanBannerGold;
        case 4:
            return u.japanBannerPlatinum;
        case 5:
            return u.japanBannerDiamond;
        default:
            return u.japanBannerBronze
    }
}

function no(s) {
    switch (s) {
        case 1:
            return "#ffac04";
        case 2:
            return "#fff8a1";
        default:
            return "#ff5ac4"
    }
}

function ao(s) {
    switch (s) {
        case 2:
            return "#0e7a3d";
        case 3:
            return "#eb9106";
        case 4:
            return "#c16959";
        case 5:
            return "#690ee0";
        default:
            return "#653120"
    }
}

function oo(s) {
    switch (s) {
        case 2:
            return "#11AD3F";
        case 3:
            return "#FF7121";
        case 4:
            return "#C16959";
        case 5:
            return "#6A0FE0";
        default:
            return "#D8733A"
    }
}

function io(s) {
    switch (s) {
        case 2:
            return ["#0ba543", "#046c2a"];
        case 3:
            return ["#e0c211", "#d57210"];
        case 4:
            return ["#f6b39f", "#be6655"];
        case 5:
            return ["#b471ff", "#940dff"];
        default:
            return ["#ff9e6a", "#ac4c19"]
    }
}

function ro(s) {
    const {
        t: e
    } = o.useTranslation();
    switch (s) {
        case 2:
            return e("Silver");
        case 3:
            return e("Gold");
        case 4:
            return e("Platinum");
        case 5:
            return e("Diamond");
        default:
            return e("Bronze")
    }
}

function Ao(s) {
    switch (s) {
        case 1:
            return "SUPER SPIN";
        case 2:
            return "MEGA SPIN";
        default:
            return "LUCKY SPIN"
    }
}

function co() {
    const s = o.useSetting();
    return (e, t) => {
        const a = Number(e || "0");
        return a >= 1e10 ? new o.Decimal(e).div(1e9).toFixed(0, o.Decimal.ROUND_DOWN) + "B" : a >= 1e7 ? new o.Decimal(e).div(1e6).toFixed(0, o.Decimal.ROUND_DOWN) + "M" : a >= (t ? 1e4 : 1e5) ? new o.Decimal(e).div(1e3).toFixed(0, o.Decimal.ROUND_DOWN) + "K" : o.utils.numberWithCommas(s.localeCurrencyName, a)
    }
}
const la = "/modules/bonus/win-1d4c0d41.mp3",
    ua = "/modules/bonus/bigwin-2f482a51.mp3",
    da = "/modules/bonus/update-83b4f99b.mp3",
    pa = "/modules/bonus/oldsounds-b8fb8ba1.mp3",
    {
        useProxy: ma,
        proxy: ga
    } = o.valtio,
    L = {
        win: la,
        bigwin: ua,
        update: da
    };
class fa extends o.State {
    constructor(t) {
        super(t);
        B(this, "initData");
        B(this, "animateResolve", () => {});
        B(this, "soundSprites");
        B(this, "sounds");
        this.initData = this.initFn(), o.app.on("get-spin-rain-or-tip", () => {
            this.getUserInfo()
        });
        let a = {};
        Object.keys(L).map(n => (a[n] = new j({
            src: L[n]
        }), n)), this.sounds = g({}, a), this.soundSprites = new j({
            src: pa,
            sprite: {
                SpinOpen: [0, 1979],
                SpinAndBonus: [1979, 8268],
                Click: [10247, 306],
                Collect: [10553, 601]
            }
        })
    }
    async initFn() {
        const t = new Date().getTime(),
            a = await o.http.get("/activity/lucky/spin/settings/");
        this.setState({
            updateTime: t,
            tabs: this.getTabs(0),
            spinOptions: a.sections,
            version: a.version,
            fiat: a.fiat
        }), await o.accountStore.initData, o.accountStore.state.login ? await this.getUserInfo() : o.accountStore.waitLogin().then(() => {
            this.getUserInfo()
        });
        try {
            await o.utils.loadImage(ca(this.state.spinLevel))
        } catch (n) {
            console.log(n)
        }
        return this.state
    }
    async spinAnimateEnd() {
        return new Promise(t => {
            this.animateResolve = t
        })
    }
    setSpinAnimateEnd() {
        this.animateResolve()
    }
    async getUserInfo() {
        try {
            if (this.state.fetching) return;
            this.setState({
                fetching: !0,
                levelUpdated: !1
            });
            const t = await o.http.get("/activity/lucky/spin/info/"),
                a = localStorage.getItem("spinUpdated"),
                n = Number(a) !== t.level && t.levelUpdated;
            let r = n ? t.level - 1 : t.level,
                A = {
                    spinLoading: n,
                    spinLevel: r,
                    tabs: this.getTabs(r),
                    freeTimes: t.leftTimes,
                    fetching: !1
                };
            n && setTimeout(() => {
                localStorage.setItem("spinUpdated", `${t.level}`), A.levelUpdated = n
            }, 1500), this.setState(g({}, A))
        } catch (t) {
            o.notify(t)
        }
    }
    async levelUpdateFinished() {
        this.setState({
            levelUpdated: !1,
            spinLevel: this.state.spinLevel + 1,
            tabs: this.getTabs(this.state.spinLevel),
            spinLoading: !1,
            updateFinished: !0
        })
    }
    reset() {
        this.setState({
            spinLoading: !1,
            levelUpdated: !1
        })
    }
    getTabs(t) {
        return t <= 2 ? [t, 3, 5] : t <= 4 ? [2, t, 5] : [2, 4, t]
    }
    getTab(t) {
        return this.state.tabs.findIndex(a => a === t)
    }
    playSound(t) {
        if (o.settingStore.state.soundEffectEnable) try {
            this.sounds[t].play()
        } catch (a) {}
    }
    async setNextTime() {
        const t = o.utils.serverTime(),
            a = t.getUTCFullYear(),
            n = t.getUTCMonth(),
            r = t.getUTCDate() + 1;
        this.setState({
            nextTime: Date.UTC(a, n, r, 0, 0, 0)
        })
    }
    async handleSpin() {
        return o.http.post("/activity/lucky/spin/lottery/", {
            level: this.state.spinLevel,
            version: this.state.version
        })
    }
    async getDeviceRegist() {
        if (this.state.deviceRegist != null) return new Promise(a => a(this.state.deviceRegist));
        try {
            return o.http.get("/account/device/isRegistered").then(a => {
                let n = a;
                if (a === !1) {
                    let r = Date.now() % 2 > 0 ? "A" : "B";
                    const A = window.localStorage.getItem("before-user-login-type");
                    A ? r = A : window.localStorage.setItem("before-user-login-type", r), r === "B" && (n = !0), o.app.emit("track", "PlanPopupDisplay", {
                        sf_plan_id: "bc_free_bonus_ab_test",
                        sf_plan_strategy_id: n ? -1 : 0
                    })
                }
                return this.setState({
                    deviceRegist: n
                }), n
            })
        } catch (a) {
            console.log(a)
        }
    }
}
const ha = new fa(ga({
    nextTime: 0,
    freeTimes: 0,
    spinOptions: [],
    spinLevel: 5,
    tabs: [],
    fiat: o.accountStore.state.bonusCurrencyName !== "BCD",
    totalBonus: 0,
    spinLoading: !1,
    version: 0,
    updateTime: 0,
    levelUpdated: !1,
    updateFinished: !1,
    fetching: !1,
    deviceRegist: null
}));

function lo() {
    return ma(ha.state)
}
const ba = ({
        startValue: s = 0,
        targetValue: e,
        className: t,
        decimalPlaces: a = 2,
        duration: n = 1.5,
        currency: r
    }) => {
        const A = h.useRef(null),
            l = o.env.localCurrencys.find(f => f.value === r),
            c = s.toFixed(a),
            d = o.systemStore.getAlias(r),
            m = l ? l.cs + " " + c : c + " " + d;
        return h.useEffect(() => {
            if (A.current) {
                const f = {
                        num: s
                    },
                    w = ds.to(f, {
                        num: e,
                        duration: 2,
                        delay: .5,
                        ease: "Linear.easeNone",
                        roundProps: `value,${a}`,
                        onUpdate: () => {
                            if (A.current) {
                                const b = f.num.toFixed(a);
                                A.current.textContent = l ? l.cs + " " + b : b + " " + d
                            }
                        }
                    });
                return () => {
                    w.kill()
                }
            }
        }, [s, e, a, n]), i.jsx("div", {
            className: D("scoll-num", t, Ba),
            ref: A,
            children: m
        })
    },
    Ba = "sxm7p3g",
    {
        useProxy: Da,
        proxy: ya
    } = o.valtio;
class wa extends o.State {
    constructor(t) {
        super(t);
        B(this, "rewardsCurrency", "USDFIAT");
        B(this, "rewardsList", [1, 2, 5, 6, 8, 10, 16, 20, 26, 30, 60, 100, 200, 300, 400, 500]);
        B(this, "rewardsUsd", 5);
        B(this, "target", 2);
        B(this, "currencys", ["BCD", "USDT", "USDC", "TRX", "SOL", "DOGE", "BNB", "LTC", "XML", "MATIC", "DOT"]);
        this.initFn()
    }
    initFn() {
        this.setState({
            claimed: !!localStorage.getItem("newuser_spin"),
            randomCurrencys: U.shuffle(this.currencys)
        })
    }
}
const Ca = new wa(ya({
    claimed: !1,
    randomCurrencys: []
}));

function uo() {
    return Da(Ca.state)
}

function po(s, e = 2) {
    const t = Math.floor(Math.log10(Math.abs(s)) + 1),
        a = e - t;
    return Math.round(s * Math.pow(10, a)) / Math.pow(10, a)
}

function mo(s, e) {
    let t = "";
    if (e.toLocaleLowerCase() === "crypto") return "Crypto";
    const a = o.walletStore.state[e];
    return e && a ? t = e : t = s, o.walletStore.state[t] && o.system.getCurrency(t).currencyType === "FIAT" ? t : "Crypto"
}

function va() {
    const s = o.usePop(),
        {
            t: e
        } = o.useTranslation();
    return i.jsxs(o.Pop, {
        className: Sa,
        children: [i.jsx("div", {
            className: "tit",
            children: e("Oh no! It appears somebody has already opened an account on your device/IP")
        }), i.jsx("div", {
            className: "txt",
            children: e("Unfortunately, you are not eligible to receive the welcome reward, but don’t worry there are more rewards to come!")
        }), i.jsx(o.Button, {
            type: "conic",
            onClick: s.close,
            children: e("OK")
        })]
    })
}
const Sa = "ss7hjz6";
async function Ta(s, e) {
    return o.http.post("/activity/reward/freeMoneyReward/", {
        currencyName: s,
        bonusAmount: e
    })
}

function Va({
    amount: s,
    currencyName: e,
    rewardId: t
}) {
    const [a, n] = o.useSetState({
        init: !0,
        error: ""
    }), {
        t: r
    } = o.useTranslation(), A = o.useSetting(), l = o.useWallet(), c = o.usePop(), d = o.useNavigate();
    if (h.useEffect(() => {
            o.http.post(`/activity/reward/receive/${t}/`).then(() => {
                n({
                    init: !0
                }), l[e] && (A.currencyName = e)
            }).catch(f => {
                n({
                    error: f.message
                })
            })
        }, []), a.error) return i.jsx(o.Pop, {
        closeable: !0,
        children: i.jsx(o.Empty, {
            children: a.error
        })
    });
    if (!a.init) return i.jsx(o.Pop, {
        className: E,
        closeable: !0,
        children: i.jsx(o.Loading, {})
    });
    const m = function(w) {
        d(w), o.app.emit("track", "free_play_click"), c.close()
    };
    return i.jsxs(o.Pop, {
        className: E,
        closeable: !0,
        children: [i.jsx(ba, {
            className: "amount",
            duration: 3e3,
            targetValue: Number(s),
            currency: e
        }), i.jsxs("div", {
            className: "tit ttu",
            children: [r("Rewards Claimed"), "!"]
        }), i.jsx("div", {
            className: "desc",
            children: r("Use this bonus money to play games now!")
        }), i.jsx(o.Button, {
            type: "conic",
            onClick: () => m(`#/newuser/gamelist/${e}`),
            children: r("Play")
        })]
    })
}

function go({
    amount: s,
    currency: e
}) {
    const {
        data: t,
        error: a
    } = o.useAsync(() => Ta(e, s));
    return a ? i.jsx(o.Pop, {
        children: i.jsx(o.Empty, {
            children: a.message
        })
    }) : t ? t.state != 0 || !t.rewardId ? i.jsx(va, {}) : i.jsx(Va, {
        amount: t.amount,
        currencyName: t.currencyName,
        rewardId: t.rewardId
    }) : i.jsx(o.Pop, {
        className: E,
        closeable: !0,
        children: i.jsx(o.Loading, {})
    })
}
const E = "svozrz2",
    {
        useProxy: ka,
        proxy: xa
    } = o.valtio,
    qa = o.utils.timeMemoize(function() {
        return o.http.post("/activity/taskhub/tasks/")
    }, {
        timeout: 1e3
    });
class ja extends o.State {
    constructor(e) {
        super(e), this.initFn()
    }
    async initFn() {
        await o.accountStore.waitLogin();
        try {
            this.setState({
                loading: !0
            }), this.setState({});
            const e = await qa();
            let t = P(g({}, e), {
                newbieTasks: e.dailyTasks.filter(n => n.newbieTask),
                unClaimedRewardMap: e.unClaimedRewardMap || {}
            });
            const a = e.weekTaskExpireTimeInMs > 0 ? e.weekTaskExpireTimeInMs + Date.now() : 0;
            t.weekTaskExpireTimeInMs = a, t.dailyTaskExpireTimeInMs = e.dailyTaskExpireTimeInMs > 0 ? e.dailyTaskExpireTimeInMs + Date.now() : 0, this.setState({
                data: g({}, t),
                cutTime: a,
                isBrlBonus: this.brlReward(e),
                loading: !1
            })
        } catch (e) {
            o.notify(e), this.setState({
                loading: !1
            })
        }
    }
    brlReward(e) {
        const t = e.dailyTasks;
        let a = !1;
        return t.map(n => {
            (n.taskId === "tid061" || n.taskId === "tid062") && (a = !0)
        }), a
    }
    updateClaimed(e) {
        if (e) {
            const t = U.cloneDeep(this.state.data),
                a = this.state.data.rewards;
            if (a.length > 0) {
                const n = a.find(r => r.rewardId === e);
                n && (n.rewardId = 0, t.rewards = [...a], this.setState({
                    data: g({}, t)
                }), o.app.emit("rewardClaim"))
            }
        }
    }
    changeUnClaimedNum(e) {
        const t = U.cloneDeep(this.state.data),
            a = g({}, this.state.data.unClaimedRewardMap),
            n = a[e];
        n && (a[e] = Math.max(n - 1, 0)), t.unClaimedRewardMap = g({}, a), this.setState({
            data: g({}, t)
        })
    }
    havePrevious(e) {
        if (e) {
            if (JSON.stringify(e) === "{}") return 0;
            let t = 0;
            for (let a in e) !(a === this.state.data.taskDate) && e[a] && e[a] > 0 && (t += e[a]);
            return t
        } else return 0
    }
}
const Ua = new ja(xa({
    cutTime: 0,
    loading: !0,
    isBrlBonus: !1,
    data: {
        rewards: [],
        rewardsAmount: 0,
        newbieTaskExpireTime: 0,
        unClaimedRewardMap: {},
        taskDate: "",
        lastTaskWeek: "",
        taskWeek: "",
        rewardCurrency: "",
        dayOfWeek: "",
        taskDates: [],
        newbieTasks: [],
        dailyTasks: [],
        weeklyTasks: [],
        dailyTaskExpireTimeInMs: 0,
        weekTaskExpireTimeInMs: 0
    }
}));

function fo() {
    return ka(Ua.state)
}
const ho = "/modules/bonus/gift-3dbd827b.png";
export {
    fo as $, ro as A, ao as B, u as C, oo as D, us as E, ha as F, Ka as G, _a as H, ca as I, Xa as J, eo as K, st as L, to as M, Ia as N, $a as O, rs as P, Ha as Q, Y as R, Za as S, so as T, co as U, Wa as V, Fa as W, io as X, uo as Y, Ca as Z, o as _, i as a, Ua as a0, ho as a1, Q as a2, N as a3, le as a4, mo as a5, po as a6, go as a7, I as a8, j as a9, Ao as aa, no as ab, ba as ac, C as b, D as c, v as d, za as e, Ma as f, Z as g, Ae as h, Oa as i, La as j, ce as k, ue as l, Ja as m, U as n, Ga as o, pe as p, Qa as q, h as r, Pa as s, O as t, Na as u, ds as v, X as w, Ya as x, Nt as y, lo as z
};