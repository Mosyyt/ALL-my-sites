...
const Jt = de.memo(function() {
    ...
    Xt = ct.once(() => C.utils.loadScript("https://accounts.google.com/gsi/client")),
    Zt = () => {
...
async function et(S) {
    const o = await C.http.post("/account/open/login-callback/", S);
    return C.app.emit("signin-success"), o
}
...
async function ti(S) {
    return S === "steam" ? await C.http.get("/account/steam/login/url/") : S === "telegram" ? `https://oauth.telegram.org/auth?bot_id=${await C.http.post("/account/open/telegram/botid/")}&origin=${encodeURIComponent(location.origin)}&request_access=write` : S === "line" ? `${location.origin}/api/account/open/line/oauth/` : ""
}
async function tt() {
    return C.http.get("/account/wallet/login/message/")
}
async function it(S) {
    const o = await C.http.post("/account/wallet/login/varify/", S);
    return C.app.emit("signin-success"), o
}
var ii = /^-?(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?$/i,
    be = Math.ceil,
...
function lt(S) {
    ...
    _.clone = lt, _.ROUND_UP = 0, _.ROUND_DOWN = 1, _.ROUND_CEIL = 2, _.ROUND_FLOOR = 3, _.ROUND_HALF_UP = 4, _.ROUND_HALF_DOWN = 5, _.ROUND_HALF_EVEN = 6, _.ROUND_HALF_CEIL = 7, _.ROUND_HALF_FLOOR = 8, _.EUCLID = 9, _.config = _.set = function(s) {
        ...
        return function(n, d, l, p, f) {
            var a, h, g, u, E, N, q, M, F = n.indexOf("."),
                z = w,
                A = m;
            for (F >= 0 && (u = $, $ = 0, n = n.replace(".", ""), M = new _(d), N = M.pow(n.length - F), $ = u, M.c = r(he(ie(N.c), N.e, "0"), 10, l, s), M.e = M.c.length), q = r(n, d, l, f ? (a = Z, s) : (a = s, Z)), g = u = q.length; q[--u] == 0; q.pop());
            if (!q[0]) return a.charAt(0);
            if (F < 0 ? --g : (N.c = q, N.e = g, N.s = p, N = o(N, M, z, A, l), q = N.c, E = N.r, g = N.e), h = g + z + 1, F = q[h], u = l / 2, E = E || h < 0 || q[h + 1] != null, E = A < 4 ? (F != null || E) && (A == 0 || A == (N.s < 0 ? 3 : 2)) : F > u || F == u && (A == 4 || E || A == 6 && q[h - 1] & 1 || A == (N.s < 0 ? 8 : 7)), h < 1 || !q[0]) n = E ? he(a.charAt(1), -z, a.charAt(0)) : a.charAt(0);
            else {
                if (q.length = h, E)
                    for (--l; ++q[--h] > l;) q[h] = 0, h || (++g, q = [1].concat(q));
                for (u = q.length; !q[--u];);
                for (F = 0, n = ""; F <= u; n += a.charAt(q[F++]));
                n = he(n, g, a.charAt(0))
            }
            return n
        }
        ...
        function s(d, l, p) {
            var f, a, h, g, u = 0,
                ...
                N = l % fe,
                q = l / fe | 0;
            for (d = d.slice(); E--;) h = d[E] % fe, g = d[E] / fe | 0, f = q * h + g * N, a = N * h + f % fe * fe + u, u = (a / p | 0) + (f / fe | 0) + q * g, d[E] = a % p;
        ...
        return function(d, l, p, f, a) {
            var h, g, u, E, N, q, M, F, z, A, V, Q, Ce, Ae, Me, ce, Ie, te = d.s == l.s ? 1 : -1,
                W = d.c,
                G = l.c;
                ...
                do {
                    if (N = 0, h = r(G, A, ce, V), h < 0) {
                        if (Q = A[0], ce != V && (Q = Q * a + (A[1] || 0)), N = se(Q / Me), N > 1)
                            for (N >= a && (N = a - 1), q = s(G, N, a), M = q.length, V = A.length; r(q, A, M, V) == 1;) N--, n(q, ce < M ? Ie : G, M, a), M = q.length, h = 1;
                        else N == 0 && (h = N = 1), q = G.slice(), M = q.length;
                        if (M < V && (q = [0].concat(q)), n(A, q, V, a), V = A.length, h == -1)
    ...
    i = function() {
        ...
        return function(p, f, a, h) {
            ...
            if (d.test(u)) p.s = isNaN(u) ? null : u < 0 ? -1 : 1;
                ...
                if (!a && (u = u.replace(s, function(E, N, q) {
                        return g = (q = q.toLowerCase()) == "x" ? 16 : q == "b" ? 2 : 8, !h || h == g ? N : E
    ...
    return e.absoluteValue = e.abs = function() {
        ...
        var n, d, l, p, f, a, h, g, u, E, N, q, M, F, z, A = this,
            V = A.c,
            Q = (s = new _(s, r)).c;
        ...
        for (F = ae, z = fe, l = E; --l >= 0;) {
            for (n = 0, N = Q[l] % z, q = Q[l] / z | 0, f = h, p = l + f; p > l;) g = V[--f] % z, u = V[f] / z | 0, a = q * g + u * N, g = N * g + a % z * z + M[p] + n, n = (g / F | 0) + (a / z | 0) + q * u, M[p--] = g % F;
            M[p] = n
        }
        ...
        if (d = l.toFixed(s, r), l.c) {
            ...
            if (h && (p = a, a = h, h = p, M -= p), a > 0 && M > 0) {
                for (p = M % a || a, u = q.substr(0, p); p < M; p += a) u += g + q.substr(p, a);
                h > 0 && (u += g + q.slice(p)), N && (u = "-" + u)
            }
        ...
        var r, n, d, l, p, f, a, h, g, u, E, N, q = this,
            M = q.c;
        if (s != null && (a = new _(s), !a.isInteger() && (a.c || a.s !== 1) || a.lt(c))) throw Error(X + "Argument " + (a.isInteger() ? "out of range: " : "not an integer: ") + B(a));
        if (!M) return new _(q);
        for (r = new _(c), g = n = new _(c), d = h = new _(c), N = ie(M), p = r.e = N.length - q.e - 1, r.c[0] = Fe[(f = p % x) < 0 ? x + f : f], s = !s || a.comparedTo(r) > 0 ? p > 0 ? r : g : a, f = P, P = 1 / 0, a = new _(N), h.c[0] = 0; u = o(a, r, 0, 1), l = n.plus(u.times(d)), l.comparedTo(s) != 1;) n = d, d = l, g = h.plus(u.times(l = g)), h = l, r = a.minus(u.times(l = r)), a = l;
        return l = o(s.minus(n), d, 0, 1), h = h.plus(l.times(g)), n = n.plus(l.times(d)), h.s = g.s = q.s, p = p * 2, E = o(g, d, p, m).minus(q).abs().comparedTo(o(h, n, p, m).minus(q).abs()) < 1 ? [g, d] : [h, n], P = f, E
    }, e.toNumber = function() {
        return +B(this)
...
const si = ct.once(function() {
    const S = C.env.isDevHost || C.env.isDev ? "t1-tozpsiwmuyjayyiw.dogcrash.game" : C.env.host;
    return C.utils.loadScript(`//${S}/metamask-sdk.js`)
});
async function ri(S) {
    await si();
...
async function Ii() {
    return await Qe.init({
        projectId: C.env.WALLET_CONNECT_PROJECT_ID,
        metadata: _i
    })
}
...
async function Ni(S, o) {
    ...
    try {
        ...
        e instanceof Error && C.notify(e.message)
    }
}
...
const vi = function({
        ...
        const {
            t
        } = C.useTranslation(), i = C.useDialog();
        return v.jsx("div", {
            className: qi,
            children: v.jsx(C.Dialog, {
    ...
    Ci = de.memo(function({
        ...
        const $ = async R => {
                const D = await C.http.get(`/account/self-exclusion/status/${R}/`),
            ...
            Y = async R => {
                ...
                try {
                    ...
                    if (D.code === C.BC.ErrorCode.SELF_EXCLUSION) {
            ...
            Z = async () => {
                ...
                try {
                    ...
                    if (R.code === C.BC.ErrorCode.SELF_EXCLUSION) {
            ...
            K = async () => {
                ...
                try {
                    ...
                    const R = await C.requestRecaptcha("login"),
                        D = await tt();
                    O(!1);
                    let {
                        signature: B,
            ...
            _ = async R => {
                y(!0), R ? O(!0) : T(!0);
                try {
                    C.app.emit("open-wallet-connect-modal"), C.app.emit("track", "third_register_click", {
                        account_type: "wallet"
                    });
                    const D = await C.requestRecaptcha("login"),
                        B = await tt();
                    T(!1), Ni(B, async (s, r) => {
                        await it({
                            signStr: s,
            ...
            ge = async () => {
                ...
                try {
                    ...
                    R ? C.notify(R) : (C.app.emit("signin-success"), t && t())
                } catch (R) {
                    if (R.code === C.BC.ErrorCode.SELF_EXCLUSION) {
                        const D = R.message.split("|").pop();
                        $(D)
        ...
        return Se.useEffect(() => {
            ...
            children: [v.jsx("button", {
                ...
                children: e && P ? v.jsx(C.Loading, {}) : v.jsx(Qt, {})
            }), v.jsx("button", {
                id: "wc_login",
                type: "button",
                ...
                disabled: m,
                onClick: () => _(!1),
                children: e && I ? v.jsx(C.Loading, {}) : v.jsx(Wt, {})
            }), i, v.jsx("button", {
                id: "ln_login",
                type: "button",
                title: "line",
